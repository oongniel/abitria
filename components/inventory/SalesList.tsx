...
import { removeSale, updateSale } from "@/lib/batch-ops";
...
export const SalesList = ({ batch, commit, onGoToStock }: { batch: Batch; commit: Commit; onGoToStock: () => void }) => {
  ...
  // A seller may correct their own sales; the owners' entries and the totals aren't theirs.
  ...
  const mine = (s: (typeof sales)[number]) => !isSeller || s.soldBy === "seller";
    ...
    return (
      <div className="panel flex flex-col items-start gap-4 p-6 md:p-8">
        <div className="flex flex-col gap-1">
          ...
          <p className="body2 max-w-prose text-ink-2">Use Sell on any perfume in Stock. Each sale lowers what’s left{isSeller ? "." : " and moves the batch toward break-even."}</p>
        </div>
  ...
  return (
    <div className="flex flex-col gap-5">
      <p className="body2 num text-ink-2">
        {isSeller ? (
          <>
            Through resellers: <span className="font-semibold text-ink">{plain(bottles(reseller))}</span> bottles. Direct: <span className="font-semibold text-ink">{plain(bottles(direct))}</span>.
          </>
        ) : (
          <>
                  ...
                  >
                      ...
                      {editable ? (
                        <div className="flex items-center gap-1">
                          <Button
                            ...
                            onClick={() => setEditing((id) => (id === s.id ? null : s.id))}
                          >
                    ...
                    {it && (
                      <Collapse open={editing === s.id}>
                        <div className="border-t border-line/70 bg-surface-2/30 p-3">
                          <SaleForm
                            batch={batch}
                            item={it}
                            initial={s}
                            ...
                            onSubmit={(patch) => {
                              commit(updateSale(batch, s.id, patch), "Sale updated.", { undo: true });
                              setEditing(null);
                            }}
                          />
                        </div>
